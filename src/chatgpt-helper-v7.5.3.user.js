...
(function () {
    ...
    // v7.5.3: ChatGPT 5.4 Thinking — 「思考中」ボタンも生成中として扱う
    const THINKING_BUTTON_RE = /^思考中$/;
    ...
    function hasThinkingButton() {
        ...
        return buttons.some((btn) => {
            const text = (btn.textContent || '').trim();
            return THINKING_BUTTON_RE.test(text) && !btn.disabled;
        });
    }
        ...
        if (canvasRefs.length > 0) {
            ...
            try {
                ...
                for (const ref of canvasRefs) {
                    // v7.5.2 P0-2: basename完全一致で候補を絞る（大文字小文字正規化）
                    const normalizedName = ref.name.toLowerCase().trim();
                    const candidates = sandboxFiles.filter(
                    ...
                    // （mapping列挙順は時系列保証なし）
                    candidates.sort((a, b) => (a.createTime || 0) - (b.createTime || 0));
                    if (candidates.length > 1) {
                        log('sandbox同名複数', `${ref.name} x${candidates.length} — create_time最新を採用`);
                    }
    ...
    function createPanel() {
        ...
        header.textContent = '🦞 v7.5.3 Status';
